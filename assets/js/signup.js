checkForm = () => {
    if ($('#fullname').val() == '' || $('#username').val() == '' || $('#password').val() == '' || $('#c_password').val() == '' || $('#gender').val() == '' ||
        $('#email').val() == '' || $('#phone').val() == '' || $('#birthday').val() == '') {
...
$(document).ready(function() {
    ...
    $('form').submit(function(event) {
        ...
        if (!checkForm()) {
            ...
            $.ajax({
                type: 'post',
                url: 'http://localhost/adminskywall/user/add',
                data: formData,
                contentType: false,
                processData: false,
                success: function(res) {
                    alert(res);
                    window.location.href = 'http://localhost/adminskywall';
                },
                error: function(err) {
                    alert(err);
                }
    ...
    $("#username").keyup(function() {
        ...
        $.ajax({
            type: 'get',
            url: 'http://localhost/adminskywall/user/checkuser/' + username,
            success: function(res) {
    ...
    $("#email").keyup(function() {
        ...
        if (ValidateEmail(email)) {
            $.ajax({
                type: 'get',
                url: 'http://localhost/adminskywall/user/checkemail/' + email,
                success: function(res) {
